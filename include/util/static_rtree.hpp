...
class StaticRTree
{
  ...
  public:
    ...
    struct TreeIndex
    {
        ...
        std::size_t offset; // Which node on this level is this (0=leftmost)
    };
  ...
  private:
     ...
     * one in each level.
     */
    range<std::uint64_t> child_indexes(const TreeIndex &parent) const
    {
        ...
        if (is_leaf(parent))
        {
            ...
            return irange(first_child_index, end_child_index);
        }
        else
        {
