...
inline void read(storage::io::FileReader &reader, util::DeallocatingVector<T> &vec)
{
    ...
    std::size_t num_blocks = std::ceil(vec.current_size / util::DeallocatingVector<T>::ELEMENTS_PER_BLOCK);
    ...
    for (auto bucket_index : util::irange<std::size_t>(0, num_blocks-1))
    {
        ...
        reader.ReadInto(vec.bucket_list[bucket_index], util::DeallocatingVector<T>::ELEMENTS_PER_BLOCK);
    }
    std::size_t last_block_size = vec.current_size % util::DeallocatingVector<T>::ELEMENTS_PER_BLOCK;
...
inline void write(storage::io::FileWriter &writer, const util::DeallocatingVector<T> &vec)
{
    ...
    {
        writer.WriteFrom(vec.bucket_list[bucket_index], util::DeallocatingVector<T>::ELEMENTS_PER_BLOCK);
    }
    std::size_t last_block_size = vec.current_size % util::DeallocatingVector<T>::ELEMENTS_PER_BLOCK;
