...
auto makeGraph(const std::vector<MockEdge> &mock_edges)
{
    ...
    for (const auto &m : mock_edges)
    {
        ...
        edges.push_back(InputEdge{m.start, m.target, EdgeBasedGraphEdgeData {SPECIAL_NODEID, 1, 1, true, false}});
        edges.push_back(InputEdge{m.target, m.start, EdgeBasedGraphEdgeData {SPECIAL_NODEID, 1, 1, false, true}});
    }
...
BOOST_AUTO_TEST_CASE(unsplitable_case)
{
    ...
    // sorted by cells:     0  1  2  5  6 10 11  7  8  9  3  4
    // permutation by cell: 0  1  2 10 11  3  4  7  8  9  5  6
    //                      x        x  x  x  x  x           x
    // permuation:          0 10  3 11  4  7  6  1  2  8  9  5
    ...
    CHECK_EQUAL_RANGE(permutation, 0, 10, 3, 11, 4, 7, 6, 1, 2, 8, 9, 5);
}
